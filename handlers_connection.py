"""Connection lifecycle for Later Connector."""
...
@chat.function(
    "connect_later",
    "Connect Later account via credentials.",
    action_type="write",
    chain_callable=True,
    event="later-connector.connect_later",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_later(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Later Connector."""
    client = LaterClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Later Account",
...
@chat.function(
    "disconnect_later",
    "Disconnect Later account.",
    action_type="write",
    chain_callable=True,
    event="later-connector.disconnect_later",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_later(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
